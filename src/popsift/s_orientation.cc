...
//                                const int            h )
// {
//     printf("Start computing all orienatations\n");
...
// Replace the compute_orientations_sycl function with this corrected version:
...
void Pyramid::orientation( const Config& conf )
{
    ...
    for( int octave=0; octave<_num_octaves; octave++ )
    {
        ...
        if( extrema_count > 0 )
        {
            ...
            // FIX: Copy extrema from host to device
    ...
    printf("DEBUG: Total extrema with orientations: %d (expected: %d)\n", h_counter, ext_total);
    
    dbuf.extrema.resize(h_counter);
    q.memcpy(dbuf.extrema.data(), d_output_extrema, h_counter * sizeof(Extremum)).wait();
    
    printf("DEBUG: First extrema after orientation computation:\n");
    for(int i = 0; i < std::min(5, h_counter); i++) {
        const Extremum& ext = dbuf.extrema[i];
        printf("  Extremum %d: pos=(%.2f,%.2f,%d) octave=%d sigma=%.2f num_ori=%d orientations=[",
               i, ext.xpos, ext.ypos, ext.lpos, ext.octave, ext.sigma, ext.num_ori);
        for(int j = 0; j < ext.num_ori; j++) {
            printf("%.3f", ext.orientation[j]);
            if(j < ext.num_ori - 1) printf(", ");
        }
        printf("]\n");
    }
    ...
    printf("Computing prefix sums for all orientations\n");
    ori_prefix_sum( _num_octaves );
    
    printf("DEBUG: After ori_prefix_sum, dct.ori_total = %d\n", dct.ori_total);
    
    printf("Done with orientation computation\n");
}